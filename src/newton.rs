...
///     array![[2. * x[0]]].into()
/// };
/// let sol = findroot::NewtonRaphson::new(f, jac).solve(&x0, &[1e-15]);
///
/// approx::assert_relative_eq!(2.0_f64.sqrt(), sol[0], max_relative=1e-15);
/// ```
pub struct NewtonRaphson<'a, Jacobian> {
    f: Box<dyn 'a + Fn(&[f64]) -> Vec<f64>>,
    jac: Box<dyn 'a + Fn(&[f64]) -> Jacobian>,
    max_iter: usize,
}

impl<'a, Jac> NewtonRaphson<'a, Jac>
where
    Jac: Jacobian,
{
    ...
    pub fn solve(&self, init: &[f64], tol: &[f64]) -> Vec<f64> {
        ...
        for _k in 0..self.max_iter {
            ...
            let jac = (self.jac)(x.as_slice().unwrap());
            x = x - Array1::from(jac.solve_jacobian(&f));
        }
        x.to_vec()
    }
}
